#' --- 
#' title: "Systematic reviews and meta-analyses on psoriasis: role of funding sources, conflict of interest, and bibliometric indices as predictors of methodological quality" 
#' author: "Juan Ruano" 
#' date: "30 Sep 2016" 
...
DB1$metaanalysis_included         <- revalue(DB1$metaanalysis_included, c("No"="No", "Metanalisis included"="Meta-analysis included"))
...
file2                             <- read.csv2("authors_metadata_SCOPUS.csv", 
                                     sep = ";", 
                                     dec= ".", 
                                     stringsAsFactors = TRUE,
                                     header = TRUE)
...
DB_merge_ordinal_regression_dataset <- na.omit(DB_merged[,c("AMSTAR_levels_2",
                                                   "page_count", 
                                                   "conflict_of_interest",
                                                   "X5.Year.Impact.Factor",
                                                   "funding_academic",
...
dpage_count                                <- data.frame(metaanalysis_included = rep(c("No", "Meta-analysis included"), each = 15),page_count_2 = rep(c(1:10), 3))
...
dconflict_of_interest_2                   <- data.frame(metaanalysis_included = rep(c("No", "Meta-analysis included"), each = 24),conflict_of_interest = rep(c(0:15), 3))
...
dArticle.Influence.Score                  <- data.frame(metaanalysis_included = rep(c("No", "Meta-analysis included"), each = 24),Article.Influence.Score = rep(c(0:15), 3))
...
dX5.Year.Impact.Factor                    <- data.frame(metaanalysis_included = rep(c("No", "Meta-analysis included"), each = 45),X5.Year.Impact.Factor = rep(c(0.5:30), 3))
...
p1                                        <- ggplot(lpp_1,aes(x = page_count_2, y = value, colour = metaanalysis_included)) +
...
p2                                        <- ggplot(lpp_2,aes(x = conflict_of_interest, y = value, colour = metaanalysis_included)) +
                                                geom_line(aes(linetype = metaanalysis_included, color = metaanalysis_included)) +
...
p3                                         <-ggplot(lpp_3, aes(x = Article.Influence.Score, y = value, colour = metaanalysis_included)) +
                                                geom_line(aes(linetype=metaanalysis_included, color=metaanalysis_included))+
                                                scale_linetype_manual(values=c("solid", "dotted"))+
...
p4                                         <-ggplot(lpp_4, aes(x = X5.Year.Impact.Factor, y = value, colour = metaanalysis_included)) +
                                                geom_line(aes(linetype=metaanalysis_included, color=metaanalysis_included))+
                                                scale_linetype_manual(values=c("solid", "dotted"))+
...
p5                                         <-ggplot(lpp_5, 
                                                aes(x = page_count_2, y = value, colour = funding_academic)) +
                                                geom_line(aes(linetype=funding_academic, color=funding_academic))+
                                                scale_linetype_manual(values=c("solid", "dotted"))+
                                                scale_color_manual(values=c('#BB0000','#BB0000'))+
                                                scale_size_manual(values=c(3, 5))+
                                                facet_grid(variable ~ .,scales="fixed")+
                                                theme(legend.position="top")+
                                                theme(legend.title=element_blank())+
...
p6                                         <-ggplot(lpp_6, aes(x = conflict_of_interest, y = value, colour = funding_academic)) +
                                                geom_line(aes(linetype=funding_academic, color=funding_academic))+
                                                scale_linetype_manual(values=c("solid", "dotted"))+
                                                scale_color_manual(values=c('#006400','#006400'))+
                                                scale_size_manual(values=c(3, 5))+
                                                facet_grid(variable ~ ., scales="fixed")+
                                                theme(legend.position="top")+
                                                theme(legend.title=element_blank())+
...
p7                                         <-ggplot(lpp_7, aes(x = Article.Influence.Score, y = value, colour = funding_academic)) +
                                                geom_line(aes(linetype=funding_academic, color=funding_academic))+
                                                scale_linetype_manual(values=c("solid", "dotted"))+
                                                scale_color_manual(values=c('#00009B','#00009B'))+
                                                scale_size_manual(values=c(3, 5))+
                                                facet_grid(variable ~ ., scales="fixed")+
                                                theme(legend.position="top")+
                                                theme(legend.title=element_blank())+
                                                theme(panel.grid.minor = element_line(colour = "grey", linetype = "dotted"))+
                                                theme(panel.grid.major = element_line(colour = "grey"))+
                                                labs(x="Article Influence Score")+
                                                scale_y_continuous(limits=c(0,1),breaks = seq(0, 1, by = 0.20))

p8                                         <-ggplot(lpp_8, aes(x = X5.Year.Impact.Factor, y = value, colour = funding_academic)) +
                                                geom_line(aes(linetype=funding_academic, color=funding_academic))+
                                                scale_linetype_manual(values=c("solid", "dotted"))+
                                                scale_color_manual(values=c('#006400','#006400'))+
                                                scale_size_manual(values=c(3, 5))+
                                                ...
                                                theme(panel.grid.minor = element_line(colour = "grey", linetype = "dotted"))+
                                                theme(panel.grid.major = element_line(colour = "grey"))+
                                                labs(x="5 Year Impact Factor")+scale_y_continuous(limits=c(0,1),breaks = seq(0, 1, by = 0.20))
